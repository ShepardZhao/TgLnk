...
router
    .post('/active/following',function(req,res,next){
        ...
        connectionPool.CRUD('INSERT INTO FOLLOW_T (UID,BID) VALUES (?,?)', [getBID, getUID], function (result) {
            if (result.success === 0) {
    ...
    .delete('/active/following',function(req,res,next){
        ...
        connectionPool.CRUD('DELETE * FROM FOLLOW_T WHERE UID=? AND BID =?',[getUID,getBID],function(result){
            if(result.success === 0){
                ...
                res.json('true','Successfully delete the record from the follow table','1');
            }
        });
